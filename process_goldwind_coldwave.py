...
ROOT_PATH = '/home/wangqi/code/windmoe/datasets/goldwind/coldwave'
...
def save_csv(data_df: pd.DataFrame, suffix: str, station: int, columns_to_save: list):
    ...
    output_file = os.path.join(OUTPUT_PATH, f'goldwind_coldwave_{station}_{suffix}.csv')
...
def main(station_id):
    ...
    print(f"--- Starting Data Preprocessing for station {station_id} ---")

    station_cap_map = {420: 149500, 225: 49500}
    ...
    measured_df['dtime'] = pd.to_datetime(measured_df['dtime'], format='%Y-%m-%d %H:%M:%S')
    ...
    joblib.dump(scaler, os.path.join(OUTPUT_PATH, f'scaler_{station_id}.joblib'))
    ...
    print("\n--- Data Preprocessing and Saving Finished ---")
