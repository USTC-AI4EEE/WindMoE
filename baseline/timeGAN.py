    ...
    def forward(self, X, T, Z, obj, gamma=1):
        ...
        if Z is not None:
            if not torch.is_tensor(Z):
                ...
                Z = Z.to(self.device, dtype=torch.float32, non_blocking=True)
...
class TimeGANTrainer:
    def __init__(self, timegan_model, args):
        self.model = timegan_model
        self.device = args.device
        self.batch_size = args.batch_size
        self.Z_dim = args.Z_dim
        self.max_seq_len = args.max_seq_len
        ...
        self.optimizer_S = torch.optim.Adam(self.model.supervisor.parameters())
    ...
    def _get_random_noise(self, batch_size):
        return np.random.uniform(low=0.0, high=1.0, size=[batch_size, self.max_seq_len, self.Z_dim])

    def train(self, data, epochs=500):
        seq_len = data.shape[1]
        temporal_info = np.full(data.shape[0], seq_len, dtype=int)
        
        dataset = TensorDataset(torch.from_numpy(data).float(), torch.from_numpy(temporal_info).long())
        dataloader = DataLoader(dataset, batch_size=self.batch_size, shuffle=True)
        
        for phase in ["autoencoder", "supervisor", "joint"]:
            if phase == "autoencoder":
                print("Phase 1: Training Autoencoder (Embedder + Recovery)")
                num_epochs_phase = epochs
            ...
            for epoch in tqdm(range(num_epochs_phase), desc=f"Phase: {phase}"):
                for X_batch, T_batch in dataloader:
                    X_batch, T_batch = X_batch.to(self.device), T_batch.to(self.device)
                    Z_batch = self._get_random_noise(X_batch.shape[0])
                    
                    if phase == "autoencoder":
                        self.optimizer_E.zero_grad()
                        E_loss, _, _ = self.model(X_batch, T_batch, Z_batch, obj="autoencoder")
                        E_loss.backward()
                        self.optimizer_E.step()
                        
                    elif phase == "supervisor":
                        self.optimizer_S.zero_grad()
                        S_loss = self.model(X_batch, T_batch, Z_batch, obj="supervisor")
                        S_loss.backward()
                        self.optimizer_S.step()

                    else: 
                        self.optimizer_G.zero_grad()
                        G_loss = self.model(X_batch, T_batch, Z_batch, obj="generator")
                        G_loss.backward()
                        self.optimizer_G.step()

                        self.optimizer_D.zero_grad()
                        D_loss = self.model(X_batch, T_batch, Z_batch, obj="discriminator")
                        D_loss.backward()
    ...
    def sample(self, n_samples):
        self.model.eval()
        with torch.no_grad():
            Z_noise = self._get_random_noise(n_samples)
            T_info = np.full(n_samples, self.max_seq_len, dtype=int)
            T_info = torch.from_numpy(T_info).long()

            generated_data = self.model(X=None, T=T_info, Z=Z_noise, obj="inference")
